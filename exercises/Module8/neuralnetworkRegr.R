...
nn.pred = predict(mod, newdata = ir2.test)
nn.pred

performance(nn.pred, measures = list(rsq, mse))    # if error > restart R
...
plot(nn.pred$data$truth, nn.pred$data$response)



fimp=generateFeatureImportanceData(task=nn.task,
                                   learner=nn.lrn)
